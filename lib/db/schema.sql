...
-- =====================================================
-- 2. Single function for updated_at triggers
-- =====================================================
CREATE OR REPLACE FUNCTION update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
...
-- =====================================================
-- 3. Create tables (order matters due to foreign keys)
-- =====================================================
...
-- Drop dependent tables if they exist (order matters)
DROP TABLE IF EXISTS registration_audit CASCADE;
DROP TABLE IF EXISTS registrations CASCADE;
DROP TABLE IF EXISTS insurance_details CASCADE;
DROP TABLE IF EXISTS medical_history CASCADE;
...
-- Insurance Details table (no ABHA/Aadhaar columns)
CREATE TABLE IF NOT EXISTS insurance_details (
    id SERIAL PRIMARY KEY,
    patient_id BIGINT NOT NULL UNIQUE,
    insurance_provider VARCHAR(100),
...
-- View registration_details (without missing columns)
CREATE OR REPLACE VIEW registration_details AS
SELECT 
    p.patient_id,
    p.full_name_en,
...
-- Prescriptions table (pharmacy-linked)
CREATE TABLE IF NOT EXISTS prescriptions (
  id SERIAL PRIMARY KEY,
  patient_id BIGINT REFERENCES patients(patient_id),
  doctor_id INTEGER REFERENCES doctors(id),
...
ALTER TABLE sales ADD COLUMN IF NOT EXISTS prescription_id INTEGER REFERENCES prescriptions(id);
...
CREATE TABLE IF NOT EXISTS stock_adjustments (
    ...
    previous_quantity INTEGER NOT NULL,
    new_quantity INTEGER NOT NULL,
    created_by INTEGER REFERENCES users(id) NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS appointments (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    patient_id BIGINT,  -- changed to BIGINT to match patients.patient_id
    doctor_id INTEGER NOT NULL,
    appointment_date DATE NOT NULL,
    appointment_time VARCHAR(20) NOT NULL,
    consultation_type VARCHAR(20) CHECK (consultation_type IN ('in-person','teleconsultation')),
    notes TEXT,
...
CREATE TRIGGER update_visit_details_updated_at
...
-- Doctor's prescriptions (per appointment) – renamed to avoid conflict
CREATE TABLE IF NOT EXISTS  prescriptions (
  id SERIAL PRIMARY KEY,
  appointment_id UUID NOT NULL REFERENCES appointments(id) ON DELETE CASCADE,
  medication_name VARCHAR(255) NOT NULL,
